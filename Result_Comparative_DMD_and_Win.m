...
%% Data generation
e_kinematic = line(t(1:length(error1(1,:))),error1(1,:));
set(e_kinematic, 'LineStyle', '-', 'Color', C16, 'LineWidth', 1.4*lw);
e_dynamic = line(t(1:length(error2(1,:))),error2(1,:));
set(e_dynamic, 'LineStyle', '-', 'Color', C5, 'LineWidth', 1.4*lw);
...
%% Legend nomeclature
hLegend_10 = legend([e_kinematic, e_dynamic],{'$||\tilde{\mathbf{\mu}}||_{DMDc}$','$||\tilde{\mathbf{\mu}}||_{Windowed~DMDc}$'},'fontsize',12,'interpreter','latex','Color',[255 255 255]/255,'Location','northwest','NumColumns',1,'TextColor','black');
...
axes('Position',[0.237 0.83 .2 .1]);
index_zoom_1 = (t>=20) & (t<=30);
error_kineamtic_zoom = line(t(index_zoom_1),error1(index_zoom_1));
error_dynamic_zoom = line(t(index_zoom_1),error2(index_zoom_1));


set(error_kineamtic_zoom, 'LineStyle', '-', 'Color', C16, 'LineWidth', lw*0.8);
set(error_dynamic_zoom, 'LineStyle', '-', 'Color', C5, 'LineWidth', lw*0.8);
...
legend({'$DMDc$','$Windowed~DMDc$'},'fontsize',10,'interpreter','latex','Color',[255 255 255]/255,'TextColor','black')

set(gca,'ticklabelinterpreter','latex',...
        'fontsize',fontsizeTicks)
set(gca,'XTick',[1 2 3 4],'XTickLabel',{'$ISE_{x}$','$ISE_y$','$ISE_{z}$','$ISE_{\psi}$'});
hTitle_1 = title({'$\textrm{(b)}$'},'fontsize',12,'interpreter','latex','Color',C18);

hYLabel_9 = ylabel('$\textrm{ISE}$','fontsize',12,'interpreter','latex', 'Color',C18);
